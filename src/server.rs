...
pub fn start_server(
  ...
  let server = HttpServer::new(move || {
    ...
    App::new()
      ...
      .app_data(web::FormConfig::default().error_handler(|err, _req| ApiError::BadRequest(err.to_string()).into()))
      .app_data(web::PathConfig::default().error_handler(|err, _req| ApiError::BadRequest(err.to_string()).into()))
      .app_data(web::QueryConfig::default().error_handler(|err, _req| ApiError::BadRequest(err.to_string()).into()))
      .app_data(web::JsonConfig::default().error_handler(|err, _req| ApiError::BadRequest(err.to_string()).into()))
      .configure(|app| {
        app.service(
          scope("/api")
